...
Операторы `==` и `!=` допустимы для всех типов кроме string.

## 3. Ветвления if … else

1. Условие должно быть **числовым выражением** (`numeric_expression`).
   Результат приводится к истине/лжи по правилу §1.

2. При истинном условии выполняется блок сразу после `if`,
   при ложном — блок после `else` (если он есть).
...
* Shadowing разрешён.
* Зарезервированные имена (`fn`, `let`, `if`, `while`, `true`, `false`, `int`, `float`, `string`, `void`, `Pi`, `Euler`, `print`, `input`, `concat`, `substr`) нельзя использовать как идентификаторы.
...
* Вызов функции как инструкция разрешён даже при игнорировании результата.

## 7. Возврат из функции (return)

* Функции `int/float/string` обязаны иметь `return` на всех путях.
* Функции `void` и `main` могут завершаться без `return` (неявный void).

## 8. Рекурсия

* Полностью поддерживается прямая и взаимная рекурсия.
...
* `++ / --` (префиксные и постфиксные) разрешены как отдельные инструкции.

## 10. Встроенные функции

| Функция                    | Семантика                                           |
| -------------------------- | --------------------------------------------------- |
| `print(s: string)`         | Вывод строки + \n                                   |
| `input(var)`               | Читает строку из stdin и присваивает переменной var |
| `concat(a, b)`             | Конкатенация строк                                  |
| `substr(s, i, n)`          | Подстрока                                           |
| `int(x), float(x), str(x)` | Явные преобразования типов                          |
