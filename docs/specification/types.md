...
| `string` | неизменяемая Unicode-строка, хранение UTF-16        |
...
- **itos(i: int): string** - преобразует целочисленное значение в строку.
- **ftos(f: float, p: int): string** - преобразует вещественное число в строку с фиксированным количеством знаков после запятой.
- **ftoi(f: float): int** - преобразует вещественное число в целое путём отбрасывания дробной части.
- **itof(i: int): float** - преобразует целочисленное значение в вещественное.
- **sconcat(s1: str, s2: str): str** - конкатенирует две строки и возвращает результат.
- **strlen(s: string): int** - возвращает длину строки в символах.
- **substr(s: str, from: int, to: int): str** - возвращает подстроку строки, начиная с позиции `from` длиной `to`.
- **stoi(s: str): int** - преобразует строковое представление целого числа в тип `int`.
- **stof(s: str): float** - преобразует строковое представление вещественного числа в тип `float`.
...
-   Не поддерживаются суррогатные пары символов
------------------------------------------------------------------------
